...
class PriorRatio:
    ...
    def __init__(self, numerator, denominator):
        ...
        numerator_subpriors = _get_subpriors(numerator)
        denominator_subpriors = _get_subpriors(denominator)
        _remove_matching_items(numerator_subpriors, denominator_subpriors)
        self._numerator_subpriors = numerator_subpriors
        self._denominator_subpriors = denominator_subpriors
    ...
    def ln_prior_ratio(self, **parameters):
        ...
        **parameters
            ...
            denominator.
        """
        lnp_numerator = sum(_standard_lnprior(prior, parameters)
                            for prior in self._numerator_subpriors)

        lnp_denominator = sum(_standard_lnprior(prior, parameters)
    ...
    while i < len(list1):
        ...
        for j, item2 in enumerate(list2):
            if _deep_eq(item1, item2):
                del list1[i]  # Remove from list1
                del list2[j]  # Remove from list2
...
def _standard_lnprior(prior, parameters):
    """Log prior density in standard coordinates."""
    standard_parameters = {par: parameters[par]
                           for par in prior.standard_params}
    conditioned_on = {par: parameters[par] for par in prior.conditioned_on}

    sampled_parameters = prior.inverse_transform(**standard_parameters,
                                                 **conditioned_on)
    lnp = prior.lnprior(**sampled_parameters, **conditioned_on)
    lnj = prior.ln_jacobian_determinant(**standard_parameters,
                                        **conditioned_on)
    return lnp + lnj
